...
\section[reference.3dplots]{Three Dimensional Plot Types}
\label{sec:3d}
...
\begin{addplot3generic}
    ...
    into $\mathbb{R}^3$ and the latter one a map from two dimensions to $\mathbb{R}^3$. Here,
...
\begin{addplot3operation}[]{coordinates}{\marg{coordinate list}}
    ...
    described in all detail at~\ref{pgfplots:addplot:coordinates}:
...
\begin{addplot3operation}[]{file}{\marg{name}}
    ...
    requested, see the documentation of \verbpdfref{\addplot file} at~\ref{pgfplots:addplot:file} for details.
...
\end{codeexample}
    ...
    Matlab\textregistered{} displays discrete functions with matrices).
\end{pgfplotskey}
...
\labelprefix\label{cmd:addplot3:expr}
    ...
    \mathbb{R}$. The interval $[x_1,x_2]$ is determined using the |domain| key, for
...
(compare Section~\ref{sec:colormap:input:format}). The common format is
...
\end{codeexample}
    ...
    examples for the |ternary| lib at~\ref{page:ternary:contour}.
...
\end{codeexample}
    ...
    \begin{pgfplotskey}{mesh input=\mchoice{lattice,patches,image}}
        ...
        described at the beginning of this section and requires a |mesh/ordering| and perhaps
